...
from hummingbot.strategy_v2.executors.grid_executor.data_types import GridLevelStates
...
class TestComboExecutor(IsolatedAsyncioWrapperTestCase, LoggerMixinForTest):
    def setUp(self) -> None:
        super().setUp()
    ...
    def create_mock_strategy():
        ...
        type(strategy).trading_pair = PropertyMock(return_value="ADA-USDT")
        n_orders = 30
        ...
        type(connector).trading_rules = PropertyMock(return_value={"ADA-USDT": TradingRule(trading_pair="ADA-USDT",
    ...
    def get_grid_executor_from_config(self, config: ComboExecutorConfig):
        executor = ComboExecutor(self.strategy, config, self.update_interval)
        self.set_loggers(loggers=[executor.logger()])
        return executor

    @patch.object(ComboExecutor, "get_price", MagicMock(return_value=Decimal("0.95475")))
    async def test_control_task_grid_open_orders(self):
        config = ComboExecutorConfig(
            id="test",
            timestamp=123,
            side=TradeType.BUY,
            connector_name="binance",
            trading_pair="ADA-USDT",
            start_price=Decimal("0.3819000000000000211997086552"),
            end_price=Decimal("0.95475"),
            total_amount_quote=Decimal("180"),
            min_spread_between_orders=Decimal("0.02"),
            min_order_amount_quote=Decimal("6"),
            order_frequency=1.0,
            max_open_orders=1,
        ...
        self.assertEqual(len(executor.grid_levels), 30)  # Based on config parameters
        executor.update_grid_levels()
        self.assertEqual(len(executor.levels_by_state[GridLevelStates.OPEN_ORDER_PLACED]), 1)
        ...
        self.assertEqual(first_level.amount_quote, Decimal("6"))

    # @patch.object(ComboExecutor, "get_in_flight_order")
    @patch.object(ComboExecutor, "get_price")
    async def test_grid_take_profit_condition(self, get_price_mock):#, get_in_flight_order_mock):
        get_price_mock.return_value = Decimal("0.95475")
        config = ComboExecutorConfig(
            id="test",
            timestamp=123,
            side=TradeType.BUY,
        ...
        # get_price_mock.return_value = Decimal("0.9738552")
        # executor.update_metrics()
        # self.assertTrue(executor.take_profit_condition())


        get_price_mock.return_value = Decimal("0.7374620689655172401971142415")
        executor.update_metrics()
        executor.realized_pnl_quote = 40
        executor.position_pnl_quote = 5
        executor.position_size_quote = 10
        self.assertFalse(executor.take_profit_condition())
        # Test whem pngl gets to 5
        # executor._status = RunnableStatus.RUNNING
        # get_price_mock.return_value = Decimal("0.95475")

        # executor.grid_levels[len(executor.grid_levels)-1].active_open_order = TrackedOrder("OID-BUY-1")
        # order = InFlightOrder(
        ...
        #     amount=Decimal("6"),
        #     price=Decimal("0.95475"),
        #     creation_timestamp=1640001112.223,
        #     initial_state=OrderState.FILLED
        # )
        # order.executed_amount_base = Decimal("0.08")
        # order.executed_amount_quote = Decimal("6")
        # executor.grid_levels[len(executor.grid_levels)-1].active_open_order.order = order
        # executor.update_metrics()
        # await executor.control_task()
        # executor.update_grid_levels()

        # get_price_mock.return_value = Decimal("0.932")
        # executor.grid_levels[len(executor.grid_levels)-2].active_open_order = TrackedOrder("OID-BUY-2")
        # order = InFlightOrder(
        ...
        #     trade_type=TradeType.BUY,
        #     amount=Decimal("6"),
        #     price=Decimal("0.9349965517241379196229214077"),
        #     creation_timestamp=1640001112.223,
        #     initial_state=OrderState.FILLED
        # )
        # order.executed_amount_base = Decimal("0.08")
        # order.executed_amount_quote = Decimal("6")
        # executor.grid_levels[len(executor.grid_levels)-2].active_open_order.order = order
        ...
        # get_price_mock.return_value = Decimal("0.96")
        # executor = self.get_grid_executor_from_config(config)

        # executor.grid_levels[len(executor.grid_levels)-1].active_open_order = TrackedOrder("OID-BUY-1")
        # self.create_order("OID-BUY-1", TradeType.BUY, get_in_flight_order_mock, get_price_mock, executor)
        # self.fill_order("OID-BUY-1", TradeType.BUY, get_in_flight_order_mock, get_price_mock, executor)
        # executor.grid_levels[len(executor.grid_levels)-1].active_close_order = TrackedOrder("OID-SELL-1")
        # self.complete_order("OID-BUY-1", get_in_flight_order_mock, get_price_mock, executor)
        # self.create_order("OID-SELL-1", TradeType.SELL, get_in_flight_order_mock, get_price_mock, executor)
        # event = BuyOrderCreatedEvent(
        #     timestamp=1234567890,
        #     order_id="OID-BUY-1",
        #     base_asset="ETH",
        #     quote_asset="USDT",
        #     base_asset_amount=Decimal("0.08"),
        #     quote_asset_amount=Decimal("6"),
        #     order_type=OrderType.LIMIT,
        ...
        print(f"take_profit_condition: {executor.mid_price}")
        print(f"PNLPCTG: {executor.get_net_pnl_pct()}")
        print(f"PRICES: {executor.grid_levels}")

    def create_order(self, trade_type, order_id, get_in_flight_order_mock, get_price_mock, executor):
        get_in_flight_order_mock.return_value = InFlightOrder(
            ...
            amount=Decimal("6"),
            price=get_price_mock.return_value,
            creation_timestamp=1640001112.223,
            initial_state=OrderState.OPEN
        )
        event = BuyOrderCreatedEvent(
    ...
    def fill_order(self, trade_type, order_id, get_in_flight_order_mock, get_price_mock, executor):
        get_in_flight_order_mock.return_value = InFlightOrder(
            client_order_id=order_id,
            exchange_order_id="EOID4",
            trading_pair="ADA-USDT",
            ...
            trade_type=trade_type,
            amount=Decimal("6"),
            price=Decimal("0.9547499999999999875655021243"),
            creation_timestamp=1640001112.223,
            initial_state=OrderState.FILLED
        )
    ...
    def complete_order(self, order_id, get_in_flight_order_mock, get_price_mock, executor):
        get_in_flight_order_mock.return_value = InFlightOrder(
            client_order_id=order_id,
            exchange_order_id="EOID4",
            trading_pair="ADA-USDT",
    ...
    # @patch.object(ComboExecutor, "get_price", MagicMock(return_value=Decimal("100")))
    ...
    # @patch.object(ComboExecutor, "_sleep")
    # @patch.object(ComboExecutor, "get_price")
    ...
    # @patch.object(ComboExecutor, "get_in_flight_order")
    # @patch.object(ComboExecutor, "get_price", return_value=Decimal("100"))
    ...
    # @patch.object(ComboExecutor, 'adjust_order_candidates')
    # @patch.object(ComboExecutor, "get_price")
