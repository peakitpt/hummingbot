...
class ComboConfig(ControllerConfigBase):
    ...
    leverage: int = Field(default=5, client_data=ClientFieldData(is_updatable=True))
    position_mode: PositionMode = PositionMode.HEDGE

    # Boundaries
    connector_name: str = Field(default="binance_perpetual")
    trading_pair: str = Field(default="PNUT-USDT")
    side: TradeType = Field(default=TradeType.BUY)
    ...
    grid_step: Optional[Decimal] = Field(default=Decimal("0.002"), client_data=ClientFieldData(is_updatable=True))
    min_order_value: Optional[Decimal] = Field(default=Decimal("5"), client_data=ClientFieldData(is_updatable=True))
    ...
    # Risk Management
    # take_profit: Decimal = Field(default=Decimal("0.02"), gt=0, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Position take profit (e.g. 0.02 for 2%)"))
    adr: Optional[Decimal] = Field(default = Decimal("0.00051"), client_data=ClientFieldData(is_updatable=True))

    pnl: Decimal = Field(default = Decimal("0.00"), client_data=ClientFieldData(is_updatable=True))
    ...
    def triple_barrier_config(self) -> TripleBarrierConfig:
        return TripleBarrierConfig(
            ...
            take_profit = self.grid_step, #self.take_profit,
            stop_loss=None
        )
...
class Combo(ControllerBase):
    ...
    def initialize_rate_sources(self):
        self.market_data_provider.initialize_rate_sources([ConnectorPair(connector_name=self.config.connector_name, trading_pair=self.config.trading_pair)])
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        if len(self.active_executors()) == 0:
            market_price = self.market_data_provider.get_price_by_type(self.config.connector_name, self.config.trading_pair, PriceType.MidPrice)
            ...
            if self.config.pnl > 0:
                total_amount_quote = (self.config.total_amount_quote + (self.config.pnl * self.config.reinvest_profit_prctg))
            elif self.config.pnl < 0:
                total_amount_quote = (self.config.total_amount_quote + (self.config.pnl * self.config.risk_reduction_prctg))
            ...
            return [CreateExecutorAction(
                ...
                executor_config = ComboExecutorConfig(
                    ...
                    limit_price = None,#self.config.limit_price,
                    side=self.config.side,
                    total_amount_quote = total_amount_quote,
                    min_spread_between_orders = self.config.grid_step,
                    min_order_amount_quote = self.config.min_order_value,
                    max_open_orders=self.config.max_open_orders,
                    max_orders_per_batch = 15, #self.config.max_orders_per_batch,
